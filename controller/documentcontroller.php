...
namespace OCA\OffiGimp\Controller;
...
use \OCA\OffiGimp\Db;
use \OCA\OffiGimp\Helper;
use \OCA\OffiGimp\Storage;
use \OCA\OffiGimp\Download;
use \OCA\OffiGimp\DownloadResponse;
use \OCA\OffiGimp\File;
use \OCA\OffiGimp\Genesis;
use \OC\Files\View;
...
class DocumentController extends Controller {
	...
	private $cache;
	private $logger;
	const ODT_TEMPLATE_PATH = '/assets/imagetemplate.jpg';
	...
	private function prepareDocuments($rawDocuments){
		//\OC::$server->getLogger()->error('DocumentController prepareDocuments');
		...
		foreach ($rawDocuments as $key=>$document) {
			...
			\OC::$server->getLogger()->error($documents[$key]);
			...
			\OC::$server->getLogger()->error($ret['urlsrc']);
	...
	public function index(){
		...
		\OC::$server->getNavigationManager()->setActiveEntry( 'offigimp_index' );
		$maxUploadFilesize = \OCP\Util::maxUploadFilesize("/");
		$response = new TemplateResponse('offigimp', 'documents', [
	...
	public function listAll(){
		//\OC::$server->getLogger()->error('DocumentController listAll');
